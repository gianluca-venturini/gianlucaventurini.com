...
I don't like the idea of using a paid editor that could one day diverge significantly from vanilla VSCode. The editor is such a fundamental part of my dev workflow: I spend a lot of time customizing settings and building muscle memory around keyboard shortcuts I would hate to overfit on features I need to pay forever to feel a productive developer while until now the editor was free. The reason I even considered switching to Cursor is that is [fully compatible with VSCode](https://docs.cursor.com/get-started/migrate-from-vscode), all my extensions just work and there's no hidden cost for moving back.
